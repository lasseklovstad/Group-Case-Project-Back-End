...
public interface FootballMatchRepository extends JpaRepository<FootballMatch, Integer> {
	...
	@Query(value = "SELECT fm.footballMatchId, fm.matchDate, fm.season.name, fm.location.name, fm.homeTeam.teamId, fm.homeTeam.name, fm.awayTeam.teamId, fm.awayTeam.name FROM FootballMatch fm")
